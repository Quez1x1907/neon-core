...
/* Фон рисуется напрямую каждый кадр — непрозрачная заливка идёт ПЕРВОЙ,
   поэтому наслоение кадров физически невозможно (класс багов «вееров» убран).
...
function drawBGContent(c){
  ...
  // маркеры зон стройки: статичный кружок с кольцом — рисуются один раз в пререндере
...
function drawPortals(ctx){
  G.paths.forEach((pts, i)=>{
    const cell = pts.find(p=>p[0]>=0 && p[0]<G.cols && p[1]>=0 && p[1]<G.rows) || pts[0];
